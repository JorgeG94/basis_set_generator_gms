submodule (basis_driver) basis_driver_b_6_31g_2df_p
  implicit none
...
contains
  ...
  module subroutine get_basis_b_6_31g_2df_p(basis_data, element_number, ilast)
    ...
    select case (element_number)
      ...
      case(POTASSIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element POTASSIUM'
        ilast = -1
        return

      case(CALCIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element CALCIUM'
        ilast = -1
        return

      case(SCANDIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element SCANDIUM'
        ilast = -1
        return

      case(TITANIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element TITANIUM'
        ilast = -1
        return

      case(VANADIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element VANADIUM'
        ilast = -1
        return

      case(CHROMIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element CHROMIUM'
        ilast = -1
        return

      case(MANGANESE)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element MANGANESE'
        ilast = -1
        return

      case(IRON)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element IRON'
        ilast = -1
        return

      case(COBALT)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element COBALT'
        ilast = -1
        return

      case(NICKEL)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element NICKEL'
        ilast = -1
        return

      case(COPPER)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element COPPER'
        ilast = -1
        return

      case(ZINC)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element ZINC'
        ilast = -1
        return

      case(GALLIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element GALLIUM'
        ilast = -1
        return

      case(GERMANIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element GERMANIUM'
        ilast = -1
        return

      case(ARSENIC)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element ARSENIC'
        ilast = -1
        return

      case(SELENIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element SELENIUM'
        ilast = -1
        return

      case(BROMINE)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element BROMINE'
        ilast = -1
        return

      case(KRYPTON)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element KRYPTON'
        ilast = -1
        return

      case(RUBIDIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element RUBIDIUM'
        ilast = -1
        return

      case(STRONTIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element STRONTIUM'
        ilast = -1
        return

      case(YTTRIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element YTTRIUM'
        ilast = -1
        return

      case(ZIRCONIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element ZIRCONIUM'
        ilast = -1
        return

      case(NIOBIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element NIOBIUM'
        ilast = -1
        return

      case(MOLYBDENUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element MOLYBDENUM'
        ilast = -1
        return

      case(TECHNETIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element TECHNETIUM'
        ilast = -1
        return

      case(RUTHENIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element RUTHENIUM'
        ilast = -1
        return

      case(RHODIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element RHODIUM'
        ilast = -1
        return

      case(PALLADIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element PALLADIUM'
        ilast = -1
        return

      case(SILVER)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element SILVER'
        ilast = -1
        return

      case(CADMIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element CADMIUM'
        ilast = -1
        return

      case(INDIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element INDIUM'
        ilast = -1
        return

      case(TIN)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element TIN'
        ilast = -1
        return

      case(ANTIMONY)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element ANTIMONY'
        ilast = -1
        return

      case(TELLURIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element TELLURIUM'
        ilast = -1
        return

      case(IODINE)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element IODINE'
        ilast = -1
        return

      case(XENON)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element XENON'
        ilast = -1
        return

      case(CESIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element CESIUM'
        ilast = -1
        return

      case(BARIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element BARIUM'
        ilast = -1
        return

      case(LANTHANUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element LANTHANUM'
        ilast = -1
        return

      case(CERIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element CERIUM'
        ilast = -1
        return

      case(PRASEODYMIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element PRASEODYMIUM'
        ilast = -1
        return

      case(NEODYMIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element NEODYMIUM'
        ilast = -1
        return

      case(PROMETHIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element PROMETHIUM'
        ilast = -1
        return

      case(SAMARIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element SAMARIUM'
        ilast = -1
        return

      case(EUROPIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element EUROPIUM'
        ilast = -1
        return

      case(GADOLINIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element GADOLINIUM'
        ilast = -1
        return

      case(TERBIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element TERBIUM'
        ilast = -1
        return

      case(DYSPROSIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element DYSPROSIUM'
        ilast = -1
        return

      case(HOLMIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element HOLMIUM'
        ilast = -1
        return

      case(ERBIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element ERBIUM'
        ilast = -1
        return

      case(THULIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element THULIUM'
        ilast = -1
        return

      case(YTTERBIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element YTTERBIUM'
        ilast = -1
        return

      case(LUTETIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element LUTETIUM'
        ilast = -1
        return

      case(HAFNIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element HAFNIUM'
        ilast = -1
        return

      case(TANTALUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element TANTALUM'
        ilast = -1
        return

      case(TUNGSTEN)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element TUNGSTEN'
        ilast = -1
        return

      case(RHENIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element RHENIUM'
        ilast = -1
        return

      case(OSMIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element OSMIUM'
        ilast = -1
        return

      case(IRIDIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element IRIDIUM'
        ilast = -1
        return

      case(PLATINUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element PLATINUM'
        ilast = -1
        return

      case(GOLD)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element GOLD'
        ilast = -1
        return

      case(MERCURY)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element MERCURY'
        ilast = -1
        return

      case(THALLIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element THALLIUM'
        ilast = -1
        return

      case(LEAD)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element LEAD'
        ilast = -1
        return

      case(BISMUTH)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element BISMUTH'
        ilast = -1
        return

      case(POLONIUM)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element POLONIUM'
        ilast = -1
        return

      case(ASTATINE)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element ASTATINE'
        ilast = -1
        return

      case(RADON)
        if(maswrk) write(iw,*) 'ERROR: 6-31G(2df,p) basis not available for element RADON'
        ilast = -1
        return
...
end submodule basis_driver_b_6_31g_2df_p